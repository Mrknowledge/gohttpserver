...
var vm = new Vue({
  ...
  methods: {
    ...
    viewConfig: function (type) {
      const typeStr = type === "user" ? `&type=${type}` : "";
      const that = this;
      $.ajax({
        url: pathJoin([location.pathname, "?op=conf" + typeStr]),
        method: "GET",
        success: function (res) {
          const resJson = jsyaml.load(res);
          if (type === "user") {
            that.userInfo =
              resJson?.users?.map((item) => {
                const userArr = Object.entries(item);
                return {
                  name: userArr?.[0]?.[0],
                  password: userArr?.[0]?.[1],
                };
              }) || [];
            ...
            console.log(resJson);
            that.authInfo = resJson;
            $("#edit-auth-modal").modal("show");
          }
        },
      });
    },
    removeUserItem: function (idx) {
      this.userInfo.splice(idx, 1);
    },
    ...
    editUserInfo: function () {
      const resTemp = this.userInfo
        ?.filter((item) => !!item?.name?.trim() && !!item?.password?.trim())
        ?.map((item) => ({
          [item.name]: item.password,
        }));
      this.setConfig("user", { users: resTemp });
    },

    removeAuthUserItem: function (idx) {
      this.authInfo?.users?.splice(idx, 1);
    },
    addAuthUserItem: function () {
      this.authInfo?.users?.push({
        email: undefined,
        token: undefined,
        show: false,
        upload: false,
    ...
    removeAccessItem: function (idx) {
      this.authInfo?.accessTables?.splice(idx, 1);
    },
    addAccessUserItem: function () {
      this.authInfo?.accessTables?.push({
        regex: undefined,
        allow: false,
      });
    },
    ...
    genEditURL: function () {
      $.ajax({
        url: pathJoin([
          location.pathname,
          encodeURIComponent(this.currentFileName),
        ]),
        method: "PUT",
        data: {
    ...
    deletePathConfirm: function (f, e) {
      e.preventDefault();
      if (!e.altKey) {
        // skip confirm when alt pressed
        if (
          !window.confirm("Delete " + location.pathname + "/" + f.name + " ?")
        ) {
          return;
        }
      }
